...
func puzzle2() {
	...
	for _, in := range input {
		...
		result := re.FindStringSubmatch(in)[1:]
		if len(result) != 4 {
			log.Fatalf("invalid result for %q: %q", in, result)
		}
		pos1, _ := strconv.Atoi(string(result[0]))
		pos1 = pos1 - 1
		pos2, _ := strconv.Atoi(string(result[1]))
		pos2 = pos2 - 1
		char := result[2]
		pass := result[3]
		if string(pass[pos1]) == char {
			if string(pass[pos2]) != char {
				num++
			}
		} else if string(pass[pos2]) == char {
			num++
		}
	}
	fmt.Println("ans: ", num)
...
func puzzle1() {
	...
	for _, in := range input {
		...
		min, _ := strconv.Atoi(string(result[0]))
		max, _ := strconv.Atoi(string(result[1]))
		char := result[2]
		pass := result[3]
		var count int
		for _, c := range pass {
